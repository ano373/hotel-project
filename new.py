...
def update_guest():
    ...
    if updateoptions.get() == "Room number" or updateoptions.get() == "number of days":
        ...
        if updateoptions.get() == "Payment method":
            if entery_updated != "card" or entery_updated != "cash":
                entery_updated.configure(bg="#EE0000")
                check = False
            ...
            try:
               ...
               # updatelist = ["Room number", 'number of days', 'Payment method']
                if updateoptions.get() == "Room number":
                    print("yes")
                    sql = "UPDATE guests SET ROOM_REG = '%s' WHERE ID = '%s'"
                    val = (int(entery_updated.get()),int(id_guest))
                    print("bitch")
                    cursor.execute(sql)
                    db.commit()
                    ...
                    sql = "UPDATE guests SET DAY_NUM = '%s' WHERE ID = '%s'"
                    val = (int(entery_updated.get()), int(id_guest))
                    cursor.execute(sql)
                    db.commit()
                    messagebox.showinfo(title='info', message='updated successfully !')
                    ...
                    sql = "UPDATE guests SET PAY = '%s' WHERE ID = '%s'"
                    val = (int(entery_updated.get()), int(id_guest))
                    cursor.execute(sql)
                    db.commit()
                    messagebox.showinfo(title='info', message='updated successfully !')
...
Button(root, text='Go back', font=("Forte", 25), bg="#9e6960", fg="#ffffff").place(relx=0, rely=0.88, height=90,
                                                                                   width=200)
btn_search = tk.Button(root)
btn_search["bg"] = "#f0f0f0"
...
btn_checkout["bg"] = "#f0f0f0"
ft = tkFont.Font(family='Times', size=10)
btn_checkout["font"] = ft
btn_checkout["fg"] = "#000000"
